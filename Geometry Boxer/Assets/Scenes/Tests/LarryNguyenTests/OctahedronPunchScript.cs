...
public class OctahedronPunchScript : MonoBehaviour{
	bool canPunch = false;
	...
	public KeyCode specialPower = KeyCode.V;
	...
	protected virtual void Update()
	{
		...
		if (!info.IsName(getUpProne) && !info.IsName(getUpSupine) && !info.IsName(fall) && anim.GetBool(onGround) && canPunch) //prevent use of your arms when you are on the ground and getting up.
		{
			...
			else  // keyboard controls
			{
				...
				} else if (Input.GetKeyDown (specialPower)) {
	...
	private void ThrowSinglePunch(Limbs limb)
	{
		...
		foreach (CharacterAnimations action in playerAnimations)
		{
			...
			if (limb == Limbs.rightArm)
			{
				...
				{
					currentAnim = action;
					punchTime = animClips [42].length * currentAnim.playTime;
					canPunch = false;
					//anim.speed = 5f;
					break;
				}
