...
public class punchScriptTemp : MonoBehaviour {

	bool canPunch = true;
	float currentPunchTime = 0.0f;
	float punchTime = 0.0f;
	float leewayTime = 0.3f;
	CharacterAnimations nextAnim;
	...
	void Start()
	{
		...
		nextAnim = InitCharacterAnimationStruct ();
	...
	protected virtual void Update()
	{
		...
		if (!canPunch) {
			...
			if (currentPunchTime >= punchTime) {
				currentPunchTime = 0.0f;
				canPunch = true;
				hasNextAction = false;
			}
			else if ((punchTime - currentPunchTime) <= leewayTime) {
				hasNextAction = true;
			}
		}
		...
		if (!info.IsName(getUpProne) && !info.IsName(getUpSupine) && !info.IsName(fall) && anim.GetBool(onGround) && (canPunch || hasNextAction)) //prevent use of your arms when you are on the ground and getting up.
		{
			...
			else  // keyboard controls
			{
				...
				} else if (canPunch && nextAnim.actionIndex != -1) {
					ThrowSinglePunch (Limbs.rightArm);
				}
			}
		}
		else
		{
			...
			//reset the queued up action if the player is not able to execute it
			if (nextAnim.actionIndex != -1)
				nextAnim = InitCharacterAnimationStruct ();
		}
	}
			...
			foreach (CharacterAnimations action in playerAnimations) 
			{
				...
				if (limb == Limbs.leftArm) {
					if (leftGrab && action.animName == "SwingProp") {
						...
						currentAnim = action;
						punchTime = animClips [42].length * 0.2f;
						canPunch = false;
						if (hasNextAction) {
				...
				if (limb == Limbs.rightArm) {
					...
					if (rightGrab && action.animName == "SwingProp") {
						...
						currentAnim = action;
						punchTime = animClips [42].length * currentAnim.playTime + 0.1f;
						canPunch = false;
						if (hasNextAction) {
							nextAnim = currentAnim;
							//hasNextAction = false;
