...
public class PunchScript : MonoBehaviour
{
        ...
        if (!info.IsName(getUpProne) && !info.IsName(getUpSupine) && !info.IsName(fall) && anim.GetBool(onGround)) //prevent use of your arms when you are on the ground and getting up.
        {
            ...
            else  // keyboard controls
            {
                ...
                if (controllingLeftArm)
                {
                    ...
                    leftArmXAxis = (Input.mousePosition.x - halfScreenWidth) / halfScreenWidth;
                    leftArmYAxis = (Input.mousePosition.y - halfScreenHeight) / halfScreenHeight;
                    MoveArm(Limbs.leftArm, leftArmXAxis, leftArmYAxis);
                }
                ...
                if (controllingRightArm)
                {
                    ...
                    //rightArmYAxis = Input.GetAxisRaw("Mouse Y"); (rawValue - min) / (max - min);
                    rightArmXAxis = (Input.mousePosition.x - halfScreenWidth) / halfScreenWidth;
                    rightArmYAxis = (Input.mousePosition.y - halfScreenHeight) / halfScreenHeight;
                    MoveArm(Limbs.rightArm, rightArmXAxis, rightArmYAxis);
                }
    ...
    private void MoveArm(Limbs limb, float xMotion, float yMotion)
    {
        ...
        Rigidbody armToMove = null;
        float zMotion = System.Math.Abs(1.0f - (xMotion + yMotion)); 
        if (limb == Limbs.leftArm)
        {
            armToMove = leftArm;
            xMotion *= -1.0f;
        ...
        Vector3 directionToMove = new Vector3(yMotion, xMotion, zMotion); //x is up, y is to the side, z is forward 
