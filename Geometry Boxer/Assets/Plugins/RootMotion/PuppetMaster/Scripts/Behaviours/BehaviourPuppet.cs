		...
		protected override void OnFixedUpdate() {
			...
			if (state != State.Unpinned) {
				...
				foreach (Muscle m in puppetMaster.muscles) {
					...
					if (hasCollidedSinceGetUp && !puppetMaster.isBlending && m.state.pinWeightMlp < 1f && m.positionOffset.sqrMagnitude > props.knockOutDistance * knockOutDistanceSqr * stateMlp) {
